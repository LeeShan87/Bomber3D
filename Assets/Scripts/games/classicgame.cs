...
public class classicgame : gameBehavior {
	...
	{this.name="classicgame";
	 ...
	 this.player.hiscore=player.playerData["hiscore_classic"];
	...
	{
		if(timer<0){StartCoroutine( death());}
	...
	{
		if(player.hiscore>player.playerData["hiscore_classic"]) StartCoroutine(GameObject.FindGameObjectWithTag("Player").GetComponent<data>().savePlayerData("hiscore_classic",player.hiscore));
		...
		{AddPortal(teglak.transform.GetChild(Random.Range(0,teglak.transform.GetChildCount())).position);}
